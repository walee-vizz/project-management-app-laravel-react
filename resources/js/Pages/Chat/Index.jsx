...
export default function Index({ auth, sessionParams, rooms, queryParams = null, session = null }) {
    ...
    const deleteUser = (user) => {
        if (confirm('Are you sure you want to delete this user?')) {
            ...
            // router.delete(route('rooms.destroy', user.id));
            router.delete(route('rooms.destroy', user.id));
        }
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        ...
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                                    ...
                                    {rooms.data.map((room, index) => (
                                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={room.id}>
                                            ...
                                            <th scope="row" className="px-6 py-4 font-medium text-gray-900 hover:underline whitespace-nowrap dark:text-white">
                                                <Link href={route('chat.room', room.id)} >
                                                    {room.name}
                                                </Link>
                                            </th>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-nowrap">
                                                {/* <Link href={route('chat.room.edit', { room: room.id })} className="mx-1 font-medium text-blue-600 dark:text-blue-500 hover:underline" >
                                                    Edit
                                                </Link>
                                                <Link onClick={e => deleteRoom(room)} className="mx-1 font-medium text-red-600 dark:text-red-500 hover:underline" >
                                                    Delete
                                                </Link> */}
                                            </td>
                                        </tr>
                            ...
                            {/* <Pagination Links={rooms.meta.links} className="mx-auto" queryParams={queryParams} /> */}
                        </div>
                    </div>
                </div>
            </div>
